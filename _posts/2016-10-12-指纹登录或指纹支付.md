 ...
 * iOS 8及以上系统版本；如果要做指纹支付，需要注意指纹库删减更换的漏洞，因此需要监测指纹库的Hash值（用于判断指纹库是否有变化），需要iOS 9以上版本。
...
在很多情况下，都需要先判断系统是否支持指纹，调用系统API`canEvaluatePolicy:`判断设备是否支持，主要处理**`4种情况`**，其他情况默认值即可。包括：
...
* 注意的是，Hash值为一串字符串，且在iOS 9之后才支持，所以如果用到此值，你App的指纹登录/支付功能需要判断系统版本>9.0
